...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password == $hashedPassword) {
        ...
        $stmt = $conn->prepare("DELETE FROM instructor_user WHERE user_Id = ?");
        $stmt->bind_param("s", $userId);
        $stmt->execute();
        $stmt->close();
        ...
        mysqli_query($conn, $delete_membership_query);
                ...
                $to = $email;
                $subject = "Instructor deleted";
                $message = "We wanted to let you know that your Fitnes Zone Intructor was deleted. If you did not perform this action, please contact us.";
